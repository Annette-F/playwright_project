...
def test_click_button(page, base_url):
    ...
    with allure.step('Нажимаем на кнопку по атрибуту name'):
        ...
        # Явное ожидание, чтобы убедиться, что перенаправление произошло
        # page.wait_for_url(f'{base_url} /button-success/?button1=', timeout=1000)
...
@allure.testcase('TMS-002', 'Тест клика на ссылку')
@allure.title('Клик на ссылку по относительному пути')
@allure.severity(allure.severity_level.NORMAL)
...
@allure.testcase('TMS-003', 'Тест ввода тескта')
...
def test_text_input(page, base_url):
    ...
    with allure.step('Вводим email'):
        ...
        email_input.click()
        email_input.fill('test@email.ru')
...
@allure.severity(allure.severity_level.NORMAL)
def test_radio_button(page, base_url):
    with allure.step('Открываем сайт'):
        page.goto(f'{base_url}/simple-html-elements-for-automation/')
...
@allure.testcase('TMS-007', 'Тест данных таблиц')
