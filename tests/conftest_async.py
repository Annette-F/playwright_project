...
async def async_browser(async_playwright_instance):
    browser = await async_playwright_instance.chromium.launch(headless=True, slow_mo=500)
    yield browser
    await browser.close()
...
async def async_page(async_browser):
    context = await async_browser.new_context(viewport={'width': 1920, 'height': 1080})
...
async def async_auth_token(async_api_request_context, config):
    ...
    print(f'Sтатус регистрации: {response.status}')
