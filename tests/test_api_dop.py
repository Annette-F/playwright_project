...
def test_login_with_invalid_credentials(api_request_context, config):
    payload = {
...
def test_response_header(api_request_context, config):
...
def test_response_time(api_request_context, config):
    ...
    assert response_time < 2  # Ответ должен прийти менее чем через 2 секунды
...
def test_contacts_pagination(api_request_context, auth_token):
    ...
    expect(response).to_be_ok()
    data = response.json()
    assert len(data) <= 5  # Проверка, что вернулось не более 5 записей
