...
async def test_async_create_contacts_api_check_ui(async_page, async_auth_token, async_api_request_context, config):
    ...
    # Обновляем страницу после создания контактов
...
async def test_async_parallel_ui_api_check(async_page, async_auth_token, async_api_request_context, config):
    ...
    # Создаем тестовый контакт проверки
    test_contact = {
        'firstName': 'Test',
        'lastName': 'Contact',
    ...
    assert response.ok, f'Failed to create contact'
    ...
    # Асинхронная функция для получения контактв из UI
    async def get_contacts_ui():
        contacts_elements = await async_page.get_by_role('row').all()
        contacts = []
        for element in contacts_elements[1:]:  # Пропускаем заголовок
    ...
    assert len(api_contacts) == len(ui_contacts), f'''
        Количество контаков не совпадает:
    ...
    for contact in api_contacts:
        ...
        assert any(
            full_name in ui_contact for ui_contact in ui_contacts), f'Contact {full_name} from API not found in UI'
